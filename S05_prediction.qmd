...
```{r load_covar}
cfg = read_configuration(version = "g_Mar")
...
plot(nowcast['.pred_presence'], main = "Nowcast March", 
...
```{r plot_class_labels}
plot(nowcast['.pred'], main = "Nowcast March Labels", 
...
```{r plot_forecast}
coast = read_coastline()
plot(forecast_2075['.pred_presence'], main = "RCP85 2075 June", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
...
write_prediction(nowcast, file = file.path(path,"g_Jul_RCP85_2020.tif"))
write_prediction(forecast_2055, file = file.path(path, "g_Jul_RCP85_2055.tif"))
write_prediction(forecast_2075, file = file.path(path, "g_Jul_RCP85_2075.tif"))

# or write them together in a "multi-layer" file?
write_prediction(rcp85, file = file.path(path, "g_Jul_RCP85_all.tif"))
```
...
```{r my_loop}
 for(mon in month.abb){
   for(approach in list("c", "g")){
     for(year in list("2020","2055","2075")){
     data = sprintf("%s_%s_%s_%s", approach, mon,"RCP45",year)
     print(data) 
     predict_stars(fitted_wflow, data, data_path("predictions")) }
   }
 }
...
```
```{r forecast}
forecast_2075 = predict_stars(wflow, covars_rcp45_2075)
forecast_2075
```
```{r plot_forecast}
coast = read_coastline()
plot(forecast_2075['.pred_presence'], main = "RCP45 2075 June", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
```{r load_2055_RCP45}
covars_rcp45_2055 = read_brickman(db |> filter(scenario == "RCP45", year == 2055, interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "Jun") 
forecast_2055 = predict_stars(wflow, covars_rcp45_2055)
forecast_2055
```
```{r bind}
rcp45 = c(nowcast, forecast_2055, forecast_2075, along = list(year = c("2020", "2055", "2075")))
```
```{r plot_hsi_series}
plot_coast = function(){
  plot(coast, col = "orange", lwd = 2, add = TRUE)
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,"g_Jun_RCP45_2020.tif"))
write_prediction(forecast_2055, file = file.path(path, "g_Jun_RCP45_2055.tif"))
write_prediction(forecast_2075, file = file.path(path, "g_Jun_RCP45_2075.tif"))

# or write them together in a "multi-layer" file?
write_prediction(rcp45, file = file.path(path, "g_Jun_RCP45_all.tif"))
```
